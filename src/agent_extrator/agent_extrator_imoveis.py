...
# ---------------------------------------------------------
# 1. Data Schema Definition (Required Output JSON)
# ---------------------------------------------------------
...
def create_extractor_agent():
    ...
    """
    # Initialize the LLM
    # Option 1: Groq API (Extremely fast, free tier available). Uses Llama 3.
    # Requires GROQ_API_KEY environment variable
    # Using Llama 3 70B Versatile
    # llm = ChatGroq(model="llama-3.3-70b-versatile", temperature=0)
    
    # Option 2: Google Gemini API (Free tier available). Uses Gemini 1.5 Flash.
    # Requires GOOGLE_API_KEY environment variable
    # Uncomment the line below to use Google instead of Groq:
    llm = ChatGoogleGenerativeAI(model="gemini-2.5-flash", temperature=0)
    
    # Apply the structured output format defined in the Pydantic classes
    structured_agent = llm.with_structured_output(ItemCategories)
    
    # ---------------------------------------------------------
    # 2. Prompt Creation focusing on extracting 100% of items
    # ---------------------------------------------------------
...
def execute_analysis(pdf_directory: str, output_json_file: str):
    ...
    # Since the loader loads page by page, we group the content by source filename
    texts_by_file = {}
    for doc in documents:
        # The 'source' key is provided by default in PyPDFLoader with the file path
    ...
    # Limit to 6 files for testing purposes to avoid API rate limits
    ...
    for file, full_text in pdf_files_to_process:
        ...
        try:
            # ---------------------------------------------------------
            # 3. Extraction to JSON Format and Categorization
            # ---------------------------------------------------------
            # Invokes the model which will return the populated object based on the full text
            # Note: If the notices are huge, the LLM token limit might be exceeded.
            # In such cases, partitioning or size limitation should be used.
            structured_result = agent.invoke({"auction_text": full_text})
            
            # Convert the pydantic model to a native dict (using model_dump for Pydantic v2)
...
if __name__ == "__main__":
    # Define the base path as the root of the current project
    # Since the script moved to src/agent_extrator, we need an extra .parent here
    ROOT_DIR = Path(__file__).resolve().parent.parent.parent
    
    # Designated folder containing the original PDFs
    PDFS_FOLDER = ROOT_DIR / "results" / "editais_pdfs"
    
    # Path for the unified output JSON file
